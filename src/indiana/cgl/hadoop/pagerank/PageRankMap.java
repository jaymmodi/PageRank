	...
	public void map(LongWritable key, Text value, Context context)
		...
		if (rrd.targetUrlsList.size() <= 0) {
			...
			//context.write(new LongWritable(rrd.sourceUrl),
				//	new Text(String.valueOf(rrd.rankValue)));
			double rankValueforUrlsInList = rrd.rankValue
					/ (double) rrd.targetUrlsList.size();
			for (int i = 0; i < rrd.targetUrlsList.size(); i++) {
				context.write(new LongWritable(rrd.targetUrlsList.get(i)),
						new Text(String.valueOf(rankValueforUrlsInList)));
